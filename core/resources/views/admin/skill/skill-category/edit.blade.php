            ...
            <div class="col-md-12">
              <div class="card card-primary card-outline">
                  ...
                  <div class="card-header">
                      <div class="card-title">{{ __('Update Skill Category') }}</div>
                  ...
                  <div class="card-body">
                      <form class="form-horizontal" action="{{ route('admin.updateScategory', $scategory->id) }}" method="post" enctype="multipart/form-data">
                          @csrf

                          <div class="form-group row">
                            <label for="name" class="col-sm-2 control-label">{{ __('Name') }} <span class="text-danger">*</span> </label>
                            <div class="col-sm-10">
                                <input type="text" name="name" id="name" class="form-control" value="{{ $scategory->name }}" placeholder="{{ __('Enter Skill Name') }}">
                                @if($errors->has('name'))
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                ...
                                <select name="skill_type" id="skill_type" class="form-control">
                                    ...
                                    <option value="line" {{ $scategory->skill_type == 'line' ? 'selected':''}}>{{ __('Line Progress') }}</option>
                                    <option value="circle" {{ $scategory->skill_type == 'circle' ? 'selected':''}}>{{ __('Circle Progress') }}</option>
                                </select>
